...
export const SLIDE_DATA: SlideData[] = [
  ...
  },
  {
    ...
    title: 'Yol Haritamız',
    listItems: [
      { text: 'Problem: "Karışıklık Duvarı" ve Yavaş Süreçler', icon: React.createElement(FaUnlink, iconProps) },
      { text: 'Felsefe: DevOps Kültürü ve İlkeleri', icon: React.createElement(FaBrain, iconProps) },
      { text: 'Mekanizma: Hareket Halindeki CI/CD İş Akışı', icon: React.createElement(FaCogs, iconProps) },
      { text: 'Kanıt: DORA Metrikleri ile Değeri Ölçmek', icon: React.createElement(FaChartLine, iconProps) },
      { text: 'Gelecek: CI/CD\'nin Evrimi ve Yeni Ufuklar', icon: React.createElement(FaRoad, iconProps) },
    ],
  },
  {
    id: 3,
  ...
  },
  {
    ...
    twoColumn: {
      left: { title: 'Sürekli Teslimat (Continuous Delivery)', content: 'Build (auto) → Test (auto) → Deploy to Stage (auto) → Deploy to Production (MANUAL)', icon: React.createElement(FaHandPaper, { className: "text-5xl text-accent" }) },
      right: { title: 'Sürekli Dağıtım (Continuous Deployment)', content: 'Build (auto) → Test (auto) → Deploy to Stage (auto) → Deploy to Production (AUTO)', icon: React.createElement(FaRobot, { className: "text-5xl", style: { color: COLORS.primary } }) },
    },
  },
  {
    id: 18,
    ...
    tableData: {
      ...
      rows: [
        [{ text: 'Plan', icon: React.createElement(FaProjectDiagram, toolIconProps) }, 'Proje yönetimi, görev takibi', { text: '', icon: React.createElement(React.Fragment, null, React.createElement(FaJira, toolIconProps), " ", React.createElement(FaTrello, toolIconProps)) }],
        [{ text: 'Code', icon: React.createElement(FaCode, toolIconProps) }, 'Kod yazma, sürüm kontrolü', { text: '', icon: React.createElement(React.Fragment, null, React.createElement(FaGitAlt, toolIconProps), " ", React.createElement(FaGithub, toolIconProps), " ", React.createElement(FaGitlab, toolIconProps), " ", React.createElement(FaBitbucket, toolIconProps)) }],
        [{ text: 'Build', icon: React.createElement(FaBoxOpen, toolIconProps) }, 'Derleme, paketleme', { text: '', icon: React.createElement(React.Fragment, null, React.createElement(FaJenkins, toolIconProps), " ", React.createElement(SiApachemaven, toolIconProps), " ", React.createElement(SiGradle, toolIconProps), " ", React.createElement(FaDocker, toolIconProps)) }],
        [{ text: 'Test', icon: React.createElement(FaVial, toolIconProps) }, 'Otomatik testler, kod kalitesi', { text: '', icon: React.createElement(React.Fragment, null, React.createElement(SiSelenium, toolIconProps), " ", React.createElement(SiJunit5, toolIconProps), " ", React.createElement(SiSonarqube, toolIconProps)) }],
        [{ text: 'Release', icon: React.createElement(FaPaperPlane, toolIconProps) }, 'Sürüm yönetimi, artifact depolama', { text: '', icon: React.createElement(React.Fragment, null, React.createElement(FaDocker, toolIconProps), " ", React.createElement('span', null, 'Nexus, Artifactory')) }],
        [{ text: 'Deploy', icon: React.createElement(FaCloudUploadAlt, toolIconProps) }, 'Dağıtım, yapılandırma yönetimi', { text: '', icon: React.createElement(React.Fragment, null, React.createElement(SiKubernetes, toolIconProps), " ", React.createElement(SiAnsible, toolIconProps), " ", React.createElement(SiTerraform, toolIconProps), " ", React.createElement(FaAws, toolIconProps)) }],
        [{ text: 'Operate', icon: React.createElement(FaServer, toolIconProps) }, 'Altyapı yönetimi, orkestrasyon', { text: '', icon: React.createElement(React.Fragment, null, React.createElement(FaDocker, toolIconProps), " ", React.createElement(SiKubernetes, toolIconProps), " ", React.createElement(FaAws, toolIconProps)) }],
        [{ text: 'Monitor', icon: React.createElement(FaChartLine, toolIconProps) }, 'İzleme, loglama, uyarı', { text: '', icon: React.createElement(React.Fragment, null, React.createElement(SiPrometheus, toolIconProps), " ", React.createElement(SiGrafana, toolIconProps), " ", React.createElement(SiDatadog, toolIconProps), " ", React.createElement(SiElasticstack, toolIconProps)) }],
      ]
    }
  },
  // Bölüm 4
  ...
  },
  {
    ...
    threeColumn: {
      items: [
        { logo: React.createElement(FaAws, { className: "text-5xl text-gray-700" }), title: 'Amazon', text: "Tam otomatik dağıtım modeliyle yeni özellikleri günler yerine dakikalar içinde canlıya alarak pazara sürüm süresini dramatik şekilde kısalttı." },
        { logo: React.createElement(FaFilm, { className: "text-5xl text-red-600" }), title: 'Netflix', text: "Mikroservisler ve tam otomasyon sayesinde günde yüzlerce dağıtım yaparak yenilik hızını ve hizmet kararlılığını artırdı." },
        { logo: React.createElement(FaEtsy, { className: "text-5xl text-orange-500" }), title: 'Etsy', text: "Sağlam bir CI/CD boru hattı ile günde onlarca dağıtım yaparak pazar ve kullanıcı geri bildirimlerine anında yanıt verme çevikliği kazandı." }
      ]
    }
  },
  // Bölüm 5
  ...
  // Bölüm 6
  {
    ...
    listItems: [
      { text: 'DevOps, araçlardan önce bir kültür ve felsefedir: İletişim ve işbirliği esastır.', icon: React.createElement(FaBrain, iconProps) },
      { text: 'CI/CD, bu felsefeyi hayata geçiren otomasyon motorudur; hız ve kaliteyi bir araya getirir.', icon: React.createElement(FaCogs, iconProps) },
      { text: 'DORA metrikleri ile başarınızı ölçün ve iş değerini kanıtlayın.', icon: React.createElement(FaChartLine, iconProps) },
      { text: 'DevSecOps, GitOps ve AIOps gibi yeniliklerle sürekli olarak gelişmeye ve öğrenmeye devam edin.', icon: React.createElement(FaBook, iconProps) },
    ]
  },
  {
    id: 28,
